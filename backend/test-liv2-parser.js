...
async function testLiv2Parser() {
  ...
  processedRawText = processedRawText.replace(/(\n\d+)\n(\d+\s*Poppik)/gi, '$1 $2');
  processedRawText = processedRawText.replace(/(\n\d+)\n\n(Poppik)/gi, '$1 $2');
  processedRawText = processedRawText.replace(/(\n\d+)\n(Poppik)/gi, '$1 $2');
  processedRawText = processedRawText.replace(/NoItemsHSN[\s\S]*?Total/gi, '');
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (/poppik/i.test(line)) {
      ...
      console.log("Fixed Full Row Text:\n", fixedFullRowText);
      ...
      if (numbersArray.length >= 6) {
        ...
        if (titleStr.length > 0 && !titleStr.toLowerCase().includes("invoice") && !titleStr.toLowerCase().includes("pvt ltd") && !titleStr.includes("account@")) {
          ...
          console.log("✅ Adding product: ", parsedProduct);
  ...
  parsedProducts.forEach((p, idx) => {
    console.log(idx + 1 + ". " + JSON.stringify(p, null, 2));
  });
}

testLiv2Parser().catch(err => console.error("Error:", err));
