...
function Purchase() {
  ...
  const fetchPurchases = async () => {
    try {
      const res = await fetch(`${API_URL}/purchase`, { headers: getAuthHeaders() })
      if (res.ok) {
  ...
  const fetchSuppliers = async () => {
    try {
      const res = await fetch(`${API_URL}/purchase/my-csa`, { headers: getAuthHeaders() })
      if (res.ok) {
        const csaData = await res.json()
        setSuppliers([{ id: csaData.id, name: csaData.name, ...csaData }])
      }
    } catch (err) {
      console.error('Failed to fetch CSA details:', err)
      setSuppliers([{ id: 'csa', name: 'My CSA', isNameOnly: true }])
    }
  }
  ...
  const submitOrder = async () => {
    try {
      ...
      const res = await fetch(`${API_URL}/purchase/my-purchases/create`, {
        method: 'POST',
        headers: {
            ...
            <label className="block text-xs font-medium text-gray-500 mb-1">Supplier</label>
              <select
                ...
                onChange={(e) => {
                  ...
                  setSelectedSupplier({ id: s?.id || null, name: e.target.value })
                }}
              >
            ...
            <div className="p-4 border-b border-gray-200 bg-gray-50 flex flex-col sm:flex-row gap-4 items-end">
              <div className="w-full md:w-1/3">
                ...
                <select
                  ...
                  onChange={(e) => {
                    const s = suppliers.find(sup => sup.name === e.target.value)
                    if (s) setSelectedSupplier({ id: s.id, name: s.name })
