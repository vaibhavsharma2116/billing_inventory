...
function SuperAdminDistributorProducts() {
  ...
  const handleDeleteDistributorProducts = async () => {
    if (!selectedDistributor) return
    const selectedDistributorName = distributors.find(c => c.id === selectedDistributor)?.name || 'this Distributor'
    
    if (!window.confirm(`WARNING: Are you sure you want to delete ALL products for ${selectedDistributorName}? This action cannot be undone!`)) return
    
    const confirmText = window.prompt(`Type "DELETE" to confirm deleting products for ${selectedDistributorName}:`)
    ...
    try {
      setLoading(true)
      const res = await fetch(`${API_URL}/superadmin/products/all?distributorId=${selectedDistributor}`, { 
        method: 'DELETE',
        headers: getAuthHeaders()
      })
      if (!res.ok) throw new Error('Failed to delete Distributor products')
      fetchProducts()
      alert(`Products for ${selectedDistributorName} have been deleted successfully.`)
    } catch (err) {
      setError('Failed to delete Distributor products')
      setLoading(false)
    }
  }
    ...
    <div className="p-4 md:p-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        ...
        <div className="flex items-center gap-3">
          {selectedDistributor && (
            <button
              onClick={handleDeleteDistributorProducts}
              ...
              title="Delete products for the currently selected Distributor filter"
            >
              <X size={20} />
              Delete Distributor Products
            </button>
          )}
          <button
          ...
          <div>
            ...
            >
              ...
              {distributors.map((distributor, index) => (
                <option key={distributor.id || `distributor-${index}`} value={distributor.id}>{distributor.name}</option>
              ))}
            </select>
          </div>
        ...
        <div className="flex flex-col md:flex-row gap-4">
          ...
          <div className="w-full md:w-64">
            ...
            >
              ...
              {distributors.map((c, index) => (
                <option key={c.id || `distributor-${index}`} value={c.id}>{c.name}</option>
              ))}
            </select>
          </div>
        </div>
...
export default SuperAdminDistributorProducts
