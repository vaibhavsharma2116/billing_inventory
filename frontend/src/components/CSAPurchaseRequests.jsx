...
function CSAPurchaseRequests() {
  ...
  const fetchRequests = async () => {
    try {
      setLoading(true)
      const res = await fetch(`${API_URL}/csa/purchase-requests`, { headers: getAuthHeaders() })
      if (res.ok) {
  ...
  const handleApprove = async (id) => {
    if (!confirm('Are you sure you want to approve this request? This will instantly transfer stock from your inventory to the requesting CSA.')) return
    
    try {
      const res = await fetch(`${API_URL}/csa/purchase-requests/${id}/approve`, {
        method: 'PUT',
  ...
  const handleReject = async (id) => {
    ...
    try {
      const res = await fetch(`${API_URL}/csa/purchase-requests/${id}/reject`, {
        method: 'PUT',
        headers: getAuthHeaders()
      })
      if (res.ok) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Incoming Order Requests</h1>
          <p className="text-sm text-gray-500">Review and approve purchase orders placed to you by other CSAs.</p>
        </div>
        <button
              ...
              {loading ? (
                ...
                requests.map((r) => (
                  <tr key={r.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                      {r.csa ? `${r.csa.name} (${r.csa.city})` : r.distributor ? `${r.distributor.companyName} (${r.distributor.city})` : 'Unknown'}
                    </td>
          ...
          <div className="bg-white rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] flex flex-col">
            ...
            <div className="p-6 flex-1 overflow-y-auto">
              <div className="mb-6 grid grid-cols-2 gap-4">
                <div>
                  <h3 className="text-sm font-medium text-gray-500">Requester Name</h3>
                  <p className="mt-1 text-lg text-gray-900">{viewRequest.csa?.name || viewRequest.distributor?.name}</p>
                </div>
                <div>
...
export default CSAPurchaseRequests
