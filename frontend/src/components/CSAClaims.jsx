...
const API_URL = `${BASE_API_URL}/csa/claims`
...
function CSAClaims() {
  ...
  const [formData, setFormData] = useState({ brandName: '', claimDetails: '', amount: '', status: 'PENDING', distributorId: '' })
  ...
  const fetchDistributors = async () => {
    try {
      const res = await fetch(`${BASE_API_URL}/csa/distributors`, { headers: getAuthHeaders() }).catch(() => null)
      if (res?.ok) {
        const data = await res.json()
        const list = Array.isArray(data) ? data : []
        setDistributors(list)
        if (!formData.distributorId && list[0]?.id) {
          setFormData(prev => ({ ...prev, distributorId: list[0].id }))
        }
      }
    } catch (err) {
      console.error('Failed to fetch CSA distributors', err)
    }
  }
  ...
  const fetchExtraMarginClaims = async () => {
    try {
      ...
      console.error('Failed to fetch CSA extra margin claims')
    }
  }
  ...
  const fetchDisplayClaims = async () => {
    try {
      ...
      console.error('Failed to fetch CSA display claims')
    }
  }
  ...
  const fetchGstReport = async () => {
    try {
      ...
      console.error('Failed to fetch CSA GST report')
    }
  }

  const openAddModal = () => {
    setEditingClaim(null)
    setFormData({ brandName: '', claimDetails: '', amount: '', status: 'PENDING', distributorId: distributors[0]?.id || '' })
    setIsModalOpen(true)
  }
  ...
  const openEditModal = (claim) => {
    ...
    setFormData({
      ...
      distributorId: claim?.distributorId || distributors[0]?.id || ''
    })
    setIsModalOpen(true)
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error('Failed to save CSA claim')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      console.error('Failed to delete CSA claim')
    }
  }
  ...
  return (
    <div className="p-4 md:p-8">
      <h1 className="text-2xl md:text-3xl font-bold text-gray-800 mb-2">CSA Claims & GST Reports</h1>
      <p className="text-sm text-gray-600 mb-6">Claims for distributors managed by this CSA, based on sales made to those distributors.</p>
      ...
      <div className="mb-6 border-b border-gray-200 overflow-x-auto">
        <div className="flex gap-1 min-w-max">
          {[{ id: 'extra-margin', label: 'Extra Margin Claims' }, { id: 'display-claims', label: 'Display Claims' }, { id: 'gst-report', label: 'GST Summary Report' }].map(tab => (
            <button
              key={tab.id}
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6">
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 md:p-6">
              ...
              <div className="text-2xl md:text-3xl font-bold text-orange-600">₹{getNumber(extraMarginClaims.totalClaimAmount).toFixed(2)}</div>
            </div>
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 md:p-6">
              <div className="text-xs md:text-sm text-gray-500 mb-1">Number of Claims</div>
              <div className="text-2xl md:text-3xl font-bold text-blue-600">{extraMarginClaims.count || 0}</div>
            </div>
          </div>
                ...
                <tbody className="divide-y divide-gray-200">
                  {Array.isArray(extraMarginClaims.claims) && extraMarginClaims.claims.map((claim, i) => (
                    <tr key={claim?.id || i} className="hover:bg-gray-50">
                      <td className="px-4 md:px-6 py-4 font-medium text-gray-800 text-sm md:text-base">{claim?.distributorName || '-'}</td>
                      ...
                      <td className="px-4 md:px-6 py-4 text-gray-600 text-sm md:text-base">{claim?.invoiceDate ? new Date(claim.invoiceDate).toLocaleDateString() : '-'}</td>
                      ...
                      <td className="px-4 md:px-6 py-4 text-right font-semibold text-orange-600 text-sm md:text-base">₹{getNumber(claim?.claimAmount).toFixed(2)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <div className="space-y-6">
          <div className="flex justify-end">
            <button
              ...
              className="bg-blue-600 hover:bg-blue-700 disabled:bg-gray-400 text-white px-4 md:px-6 py-2 rounded-lg font-medium transition"
            >
              + Add New Claim
            </button>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            ...
            <div className="overflow-x-auto">
              {Array.isArray(displayClaims) && displayClaims.length > 0 ? (
                <table className="w-full min-w-[800px]">
                  <thead className="bg-gray-50 border-b border-gray-200">
                    <tr>
                      <th className="px-4 md:px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Distributor</th>
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {displayClaims.map((claim) => (
                      <tr key={claim?.id} className="hover:bg-gray-50">
                        <td className="px-4 md:px-6 py-4 font-medium text-gray-800 text-sm md:text-base">{claim?.distributor?.companyName || claim?.distributorName || '-'}</td>
                        ...
                        <td className="px-4 md:px-6 py-4 text-gray-600 font-semibold text-sm md:text-base">₹{getNumber(claim?.amount).toFixed(2)}</td>
                        ...
                        <td className="px-4 md:px-6 py-4 text-right space-x-2">
                          <button onClick={() => openEditModal(claim)} className="text-blue-600 hover:text-blue-800 font-medium text-sm md:text-base">Edit</button>
                          <button onClick={() => handleDelete(claim?.id)} className="text-red-600 hover:text-red-800 font-medium text-sm md:text-base">Delete</button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
                ...
                <div className="px-4 md:px-6 py-10 text-center text-gray-500">No display claims created yet.</div>
              )}
            </div>
          </div>
        </div>
              ...
              <tbody className="divide-y divide-gray-200">
                {Array.isArray(gstReport) && gstReport.map((row, index) => (
                  <tr key={index} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 md:px-6 py-3 text-right text-gray-600 text-sm md:text-base">₹{getNumber(row?.sgst).toFixed(2)}</td>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto">
            <div className="px-4 md:px-6 py-4 border-b border-gray-200 flex items-center justify-between sticky top-0 bg-white">
              <h2 className="text-lg md:text-xl font-semibold text-gray-900">{editingClaim ? 'Edit Claim' : 'Add New Claim'}</h2>
            ...
            <form onSubmit={handleSubmit} className="p-4 md:p-6 space-y-4">
              <div>
                ...
                >
                  ...
                  {distributors.map(distributor => (
                    <option key={distributor.id} value={distributor.id}>{distributor.companyName || distributor.ownerName || distributor.id}</option>
                  ))}
                </select>
              </div>
              <div>
              ...
              <div className="flex justify-end space-x-3 pt-4">
                <button type="button" onClick={() => setIsModalOpen(false)} className="px-4 py-2 text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-lg font-medium transition">Cancel</button>
                <button type="submit" disabled={loading} className="px-4 py-2 bg-blue-600 hover:bg-blue-700 disabled:bg-gray-400 text-white rounded-lg font-medium transition">{loading ? 'Saving...' : (editingClaim ? 'Update' : 'Create')}</button>
              </div>
            </form>
          </div>
        </div>
