...
export default function AdminDashboard({ view = 'dashboard' }) {
  ...
  const fetchDistributors = async () => {
    try {
      ...
      const [distRes, csaRes, csaPerfRes] = await Promise.all([
        fetch(`${API_URL}/admin/distributors?${params.toString()}`, { headers: getAuthHeaders() }),
        fetch(`${API_URL}/admin/csas`, { headers: getAuthHeaders() }),
        fetch(`${API_URL}/admin/reports/csa-performance?${params.toString()}`, { headers: getAuthHeaders() })
      ])
      
      if (distRes.ok) {
        const data = await distRes.json()
        console.log('Distributors data from API:', data)
        setDistributors(data)
      }
      if (csaRes.ok) {
        const csaData = await csaRes.json()
        setCsas(csaData)
      }
      if (csaPerfRes.ok) {
        const csaPerfData = await csaPerfRes.json()
        setCsaPerformance(csaPerfData)
      }
    } catch (error) {
  ...
  const viewDistributorDetails = (distributorId) => {
    window.location.href = `/admin/distributor/${distributorId}`
  }
  ...
  // Group distributors by CSA
  const groupDistributorsByCsa = () => {
    const result = {}
    const unassigned = []
    distributors.forEach(dist => {
      if (dist.csaId) {
        if (!result[dist.csaId]) {
          result[dist.csaId] = []
        }
        result[dist.csaId].push(dist)
      } else {
  ...
  const { result: distributorsByCsa, unassigned: unassignedDistributors } = groupDistributorsByCsa()

  // Get top performers
  const sortedDistributors = [...distributors].sort((a, b) => getNum(b.totalRevenue) - getNum(a.totalRevenue))
  const topDistributors = sortedDistributors.slice(0, 5)
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-6 md:mb-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <h1 className="text-2xl md:text-3xl font-bold text-gray-800 mb-2 flex items-center gap-2">
            Admin Console <span className="text-xs md:text-sm text-blue-500 font-semibold bg-blue-100 px-2 md:px-3 py-1 rounded-full">{currentView === 'dashboard' ? 'DASHBOARD' : 'DISTRIBUTORS'}</span>
          </h1>
        </div>
        ...
        {currentView === 'dashboard' && (
          <>
              ...
              <div className="bg-white rounded-xl md:rounded-3xl p-3 md:p-6 shadow-lg hover:shadow-xl transition-all transform hover:-translate-y-1 border border-gray-100">
                <div className="flex flex-col items-center text-center">
                  <div className="w-10 h-10 md:w-16 md:h-16 bg-blue-100 rounded-full flex items-center justify-center mb-2 md:mb-4">
                    <Building2 size={20} className="md:w-8 md:h-8 text-blue-600" />
                  </div>
                  <p className="text-sm md:text-lg font-semibold text-gray-800">Total Distributors</p>
                  <p className="text-xl md:text-3xl font-bold text-blue-600 mt-2">{loading ? '...' : distributors.length}</p>
                </div>
              </div>
            ...
            {/* Top Performers - CSAs & Distributors */}
                    ...
                    topDistributors.map((dist, index) => (
                      <div key={dist.distributorId} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="font-semibold text-gray-800 truncate max-w-[180px]">{dist.companyName}</p>
                  ...
                  <tbody className="bg-white divide-y divide-gray-100">
                    ...
                    {csas.map(csa => (
                      <React.Fragment key={csa.id}>
                        ...
                        {(distributorsByCsa[csa.id] || []).map(distributor => (
                          <tr 
                        ...
                        <tr className="bg-gradient-to-r from-gray-50 to-gray-100">
                          <td colSpan="7" className="px-6 py-3">
                            <div className="flex items-center gap-2 pl-4">
                              ...
                              <div>
                                <h4 className="font-semibold text-gray-900">Unassigned</h4>
                              </div>
                              <span className="ml-auto text-xs font-semibold text-gray-700 bg-white px-2 py-1 rounded-full">
                                {unassignedDistributors.length} Distributors
                              </span>
                            </div>
                          </td>
                        </tr>
                        {unassignedDistributors.map(distributor => (
                          <tr 
                            key={distributor.distributorId}
                            className="hover:bg-gray-50 transition-colors bg-gray-50/50"
                          >
