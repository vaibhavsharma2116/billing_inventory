...
function CSAPaymentsIn() {
  ...
  const selectDistributor = async (dist) => {
    ...
    // Don't change showList here so the form stays visible
  }
  ...
  const fetchPaymentsIn = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/csa/distributors/${distId}/payments-in`, { headers: getAuthHeaders() })
      if (res.ok) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setError('')
      const distId = selectedDistributor.distributorId || selectedDistributor.id
      const res = await fetch(`${API_URL}/csa/distributors/${distId}/payments-in/create`, {
        method: 'POST',
        headers: { 
